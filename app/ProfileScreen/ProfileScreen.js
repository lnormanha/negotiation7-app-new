...
class ProfileScreen extends Component {
  constructor(props) {
    super(props);
    // props.userRequest(props.user.payload.id);
  }

  componentDidMount() {
    const {user} = this.props;
    if (user.payload?.type_account == 'premium') {
      this.getAvailablePurchasesIap();
    }
  }
  ...
  logout() {
    ...
    this.props.navigation.replace({routeName: 'LoginScreen'});
    this.props.negotiationsReset();
  }
        ...
        <ScrollView>
          ...
          {payload?.has_subscription && (
            <ProgressContainer>
              ...
              <ProgressText>
                {subscription?.subscription_data?.hasExpired ? i18n.t("expired") : `${differenceInDays(expiryDate, startDate)} ${i18n.t('days')} ${i18n.t('remaining')}`}
              </ProgressText>
            </ProgressContainer>
          )}
          ...
          <OptionItem onPress={() => this.logout()}>
            <OptionLabel>{i18n.t('logoutLabel')}</OptionLabel>
            <OptionIcon />
          </OptionItem>

          <View style={{alignItems: 'center', paddingVertical: 24, marginVertical: 30, backgroundColor: "#017cba"}}>
